...
sentiment_counts<- tweet_words%>%inner_join(nrc, by = "word")%>%
...
log_or%>% mutate(sentiment = reorder(sentiment, log_or),)%>%
  ...
  ylab("Log odds ratio for ssociation between Android and sentiment")+
  coord_flip()


android_iphone_or%>% inner_join(nrc)%>%
  filter(sentiment == "disgust" & Android+iPhone>10)%>%arrange(desc(or))
